...
try:
    df_dji = pd.read_csv("data/df_dji.csv")
...
if if_using_a2c:
    result = pd.merge(
        result, df_result_a2c, how="outer", left_index=True, right_index=True
    )
if if_using_ddpg:
    result = pd.merge(
        result,
...
result = pd.merge(result, dji, how="outer", left_index=True, right_index=True).fillna(
    method="bfill"
)
...
plt.figure()
result.plot().get_figure().savefig("experiments/results/plot.png")
